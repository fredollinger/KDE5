...
# The most important issue is that the Qt5 qmake is available via the system path.
# This qmake is then used to detect basically everything else.
# This module defines a number of key variables and macros. 
# The variable QT_USE_FILE is set which is the path to a CMake file that can be included 
# to compile Qt 5 applications and libraries.  It sets up the compilation
# environment for include directories, preprocessor defines and populates a
# QT_LIBRARIES variable.
#
# Typical usage could be something like:
#   find_package(Qt5 5.0.0 COMPONENTS QtCore QtGui QtXml REQUIRED )
#   include(${QT_USE_FILE})
...
# (see example above). 
# For compatibility with older versions of FindQt5.cmake it is also possible to
# set the variable QT_MIN_VERSION to the minimum required version of Qt5 before the 
# find_package(Qt5) command. 
...
#        If this variable is set to TRUE, FindQt5.cmake will create imported
...
#       is much more flexible, but requires that FindQt5.cmake is executed before
#       such an exported dependency file is processed.
#
# There are also some files that need processing by some Qt tools such as moc
# and uic.  Listed below are macros that may be used to process those files.
#  
#  macro QT5_WRAP_CPP(outfiles inputfile ... OPTIONS ...)
...
#
#  macro QT5_GENERATE_MOC(inputfile outputfile )
#        creates a rule to run moc on infile and create outfile.
#        Use this if for some reason QT5_WRAP_CPP() isn't appropriate, e.g.
#        because you need a custom filename for the moc file or something similar.
#
#  macro QT5_AUTOMOC(sourcefile1 sourcefile2 ... )
#        This macro is still experimental.
#        It can be used to have moc automatically handled.
#        So if you have the files foo.h and foo.cpp, and in foo.h a 
#        a class uses the Q_OBJECT macro, moc has to run on it. If you don't
#        want to use QT5_WRAP_CPP() (which is reliable and mature), you can insert
#        #include "foo.moc"
#        in foo.cpp and then give foo.cpp as argument to QT5_AUTOMOC(). This will the
#        scan all listed files at cmake-time for such included moc files and if it finds
#        them cause a rule to be generated to run moc at build time on the 
#        accompanying header file foo.h.
#        If a source file has the SKIP_AUTOMOC property set it will be ignored by this macro.
#
#  macro QT5_ADD_DBUS_INTERFACE(outfiles interface basename)
...
#
#  macro QT5_ADD_DBUS_INTERFACES(outfiles inputfile ... )
...
#        to the name of the class.
#
#  macro QT5_ADD_DBUS_ADAPTOR(outfiles xmlfile parentheader parentclassname [basename] [classname])
...
#        adaptor itself.
#
#  macro QT5_GENERATE_DBUS_INTERFACE( header [interfacename] OPTIONS ...)
...
#
#  macro QT5_CREATE_TRANSLATION( qm_files directories ... sources ... 
#                                ts_files ... OPTIONS ...)
#        out: qm_files
...
#
#  macro QT5_ADD_TRANSLATION( qm_files ts_files ... )
#        out: qm_files
#        in:  ts_files
...
#  QT5_FOUND        If false, don't try to use Qt 4.
#
...
#                   Qt5 QT_INCLUDE_DIR and QT_QTCORE_INCLUDE_DIR are
...
#
#  QT_INCLUDE_DIR              Path to "include" of Qt5
...
#  QT_LIBRARY_DIR              Path to "lib" of Qt5
#  QT_PLUGINS_DIR              Path to "plugins" for Qt5
#  QT_TRANSLATIONS_DIR         Path to "translations" of Qt5
#  QT_IMPORTS_DIR              Path to "imports" of Qt5
#  QT_DOC_DIR                  Path to "doc" of Qt5
#  QT_MKSPECS_DIR              Path to "mkspecs" of Qt5
#
#
...
#  QT_MT_REQUIRED         Qt5 is now always multithreaded
#  
# These variables are set to "" Because Qt structure changed 
# (They make no sense in Qt5)
...
IF(QT_QT_LIBRARY)
  IF(Qt5_FIND_REQUIRED)
    MESSAGE( FATAL_ERROR "Qt3 and Qt5 cannot be used together in one project.  If switching to Qt5, the CMakeCache.txt needs to be cleaned.")
  ELSE(Qt5_FIND_REQUIRED)
    IF(NOT Qt5_FIND_QUIETLY)
      MESSAGE( STATUS    "Qt3 and Qt5 cannot be used together in one project.  If switching to Qt5, the CMakeCache.txt needs to be cleaned.")
...
IF (QT5_QMAKE_FOUND  AND  Qt5::QtCore)
   # Check already done in this cmake run, nothing more to do
   RETURN()
ENDIF (QT5_QMAKE_FOUND  AND  Qt5::QtCore)
...
IF (QT_QMAKE_EXECUTABLE)
  ...
  IF (qt_version_tmp)
    ...
    # Suppport finding at least a particular version, for instance FIND_PACKAGE( Qt5 4.4.3 )
    ...
    # Support finding *exactly* a particular version, for instance FIND_PACKAGE( Qt5 4.4.3 EXACT )
...
# message(STATUS "QT_${basename}_LIBRARY: ${QT_${basename}_LIBRARY}")
  ...
  _QT5_ADJUST_LIB_VARS(QtGui)
  _QT5_ADJUST_LIB_VARS(Qt3Support)
  _QT5_ADJUST_LIB_VARS(QtAssistant)
  _QT5_ADJUST_LIB_VARS(QtAssistantClient)
  _QT5_ADJUST_LIB_VARS(QtCLucene)
  _QT5_ADJUST_LIB_VARS(QtDBus)
  _QT5_ADJUST_LIB_VARS(QtDeclarative)
  _QT5_ADJUST_LIB_VARS(QtDesigner)
  _QT5_ADJUST_LIB_VARS(QtDesignerComponents)
  _QT5_ADJUST_LIB_VARS(QtHelp)
  _QT5_ADJUST_LIB_VARS(QtMultimedia)
  _QT5_ADJUST_LIB_VARS(QtNetwork)
  _QT5_ADJUST_LIB_VARS(QtNsPlugin)
  _QT5_ADJUST_LIB_VARS(QtOpenGL)
  _QT5_ADJUST_LIB_VARS(QtScript)
  _QT5_ADJUST_LIB_VARS(QtScriptTools)
  _QT5_ADJUST_LIB_VARS(QtSql)
  _QT5_ADJUST_LIB_VARS(QtSvg)
  _QT5_ADJUST_LIB_VARS(QtTest)
  _QT5_ADJUST_LIB_VARS(QtUiTools)
  _QT5_ADJUST_LIB_VARS(QtWebKit)
  _QT5_ADJUST_LIB_VARS(QtXml)
  _QT5_ADJUST_LIB_VARS(QtXmlPatterns)
  _QT5_ADJUST_LIB_VARS(phonon)

  # platform dependent libraries
  IF(Q_WS_X11)
    _QT5_ADJUST_LIB_VARS(QtMotif)
  ENDIF(Q_WS_X11)
  IF(WIN32)
...
ELSE(QT5_QMAKE_FOUND)
   
   SET(QT_QMAKE_EXECUTABLE "${QT_QMAKE_EXECUTABLE}-NOTFOUND" CACHE FILEPATH "Invalid qmake found" FORCE)
   
   # The code below is overly complex to make sure we do not break compatibility with CMake 2.6.x
   # For CMake 2.8, it should be simplified by getting rid of QT5_INSTALLED_VERSION_TOO_OLD and 
   # QT5_INSTALLED_VERSION_TOO_NEW
   IF(Qt5_FIND_REQUIRED)
      IF(QT5_INSTALLED_VERSION_TOO_OLD)
    IF( Qt5_FIND_VERSION_EXACT )
    ...
    ENDIF( Qt5_FIND_VERSION_EXACT )
      ELSE(QT5_INSTALLED_VERSION_TOO_OLD)
      IF( Qt5_FIND_VERSION_EXACT AND QT5_INSTALLED_VERSION_TOO_NEW )
      MESSAGE(FATAL_ERROR "The installed Qt version ${QTVERSION} is too new, version ${QT_MIN_VERSION} is required")
    ELSE( Qt5_FIND_VERSION_EXACT AND QT5_INSTALLED_VERSION_TOO_NEW )
      MESSAGE( FATAL_ERROR "Qt qmake not found!")
    ENDIF( Qt5_FIND_VERSION_EXACT AND QT5_INSTALLED_VERSION_TOO_NEW )
      ENDIF(QT5_INSTALLED_VERSION_TOO_OLD)
   ELSE(Qt5_FIND_REQUIRED)
      IF(QT5_INSTALLED_VERSION_TOO_OLD AND NOT Qt5_FIND_QUIETLY)
         MESSAGE(STATUS "The installed Qt version ${QTVERSION} is too old, at least version ${QT_MIN_VERSION} is required")
      ENDIF(QT5_INSTALLED_VERSION_TOO_OLD AND NOT Qt5_FIND_QUIETLY)
   ENDIF(Qt5_FIND_REQUIRED)
