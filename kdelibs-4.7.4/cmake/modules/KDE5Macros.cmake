# for documentation look at FindKDE5Internal.cmake

# this file contains the following macros (or functions):
# KDE5_ADD_UI_FILES
# KDE5_ADD_UI3_FILES
# KDE5_ADD_KCFG_FILES
# _KDE5_SET_CUSTOM_TARGET_PROPERTY
# _KDE5_GET_CUSTOM_TARGET_PROPERTY
# KDE5_MOC_HEADERS
# KDE5_HANDLE_AUTOMOC
# KDE5_CREATE_FINAL_FILES
# KDE5_ADD_PLUGIN
# KDE5_ADD_KDEINIT_EXECUTABLE
# KDE5_ADD_UNIT_TEST
# KDE5_ADD_EXECUTABLE
# KDE5_ADD_WIDGET_FILES
# KDE5_UPDATE_ICONCACHE
# KDE5_INSTALL_ICONS
# KDE5_REMOVE_OBSOLETE_CMAKE_FILES
# KDE5_NO_ENABLE_FINAL
# KDE5_CREATE_HANDBOOK
# KDE5_ADD_APP_ICON
# KDE5_CREATE_MANPAGE
# KDE5_CREATE_BASIC_CMAKE_VERSION_FILE (function)
# KDE5_INSTALL_AUTH_HELPER_FILES
# KDE5_AUTH_INSTALL_ACTIONS
...
if(NOT COMMAND _AUTOMOC4_KDE5_PRE_TARGET_HANDLING)

   macro(_AUTOMOC4_KDE5_PRE_TARGET_HANDLING _target _srcs)
      if(MSVC)
         add_automoc4_target("${_target}_automoc" ${_srcs})
      else(MSVC)
         automoc4(${_target} ${_srcs} )
      endif(MSVC)
   endmacro(_AUTOMOC4_KDE5_PRE_TARGET_HANDLING)


   macro(_AUTOMOC4_KDE5_POST_TARGET_HANDLING _target)
      if(MSVC)
         add_dependencies(${_target} "${_target}_automoc")
      endif(MSVC)
   endmacro(_AUTOMOC4_KDE5_POST_TARGET_HANDLING)

endif(NOT COMMAND _AUTOMOC4_KDE5_PRE_TARGET_HANDLING)


macro (KDE5_ADD_KCFG_FILES _sources )
...
#           set(KDE5_KCFGC_EXECUTABLE kconfig_compiler)
#       endif (CMAKE_CROSSCOMPILING)

       # the command for creating the source file from the kcfg file
       add_custom_command(OUTPUT ${_header_FILE} ${_src_FILE}
          COMMAND ${KDE5_KCFGC_EXECUTABLE}
          ARGS ${_kcfg_FILE} ${_tmp_FILE} -d ${CMAKE_CURRENT_BINARY_DIR}/${_rel_PATH}
          MAIN_DEPENDENCY ${_tmp_FILE}
          DEPENDS ${_kcfg_FILE} ${_KDE5_KCONFIG_COMPILER_DEP} )
...
endmacro (KDE5_ADD_KCFG_FILES)


get_filename_component(KDE5_MODULE_DIR  ${CMAKE_CURRENT_LIST_FILE} PATH)

#create the implementation files from the ui files and add them to the list of sources
#usage: KDE5_ADD_UI_FILES(foo_SRCS ${ui_files})
macro (KDE5_ADD_UI_FILES _sources )
   foreach (_current_FILE ${ARGN})
      ...
      # this is done by executing the cmake script kde5uic.cmake
      add_custom_command(OUTPUT ${_header}
         COMMAND ${CMAKE_COMMAND}
         ARGS
         -DKDE5_HEADER:BOOL=ON
         -DKDE_UIC_EXECUTABLE:FILEPATH=${QT_UIC_EXECUTABLE}
         -DKDE_UIC_FILE:FILEPATH=${_tmp_FILE}
         -DKDE_UIC_H_FILE:FILEPATH=${_header}
         -DKDE_UIC_BASENAME:STRING=${_basename}
         -P ${KDE5_MODULE_DIR}/kde5uic.cmake
         MAIN_DEPENDENCY ${_tmp_FILE}
      )
      list(APPEND ${_sources} ${_header})
   endforeach (_current_FILE)
endmacro (KDE5_ADD_UI_FILES)
...
macro (_KDE5_GET_MOC_FLAGS _moc_flags)
   set(${_moc_flags})
...
endmacro(_KDE5_GET_MOC_FLAGS)


#create the implementation files from the ui files and add them to the list of sources
#usage: KDE5_ADD_UI3_FILES(foo_SRCS ${ui_files})
macro (KDE5_ADD_UI3_FILES _sources )

   _kde5_get_moc_flags(_moc_INCS)
...
# we need to run uic3 and replace some things in the generated file
   ...
   endforeach (_current_FILE)
endmacro (KDE5_ADD_UI3_FILES)

macro (_KDE5_SET_CUSTOM_TARGET_PROPERTY _target_name _property_name _property)
   string(REGEX REPLACE "[/ ]" "_" _dir "${CMAKE_CURRENT_SOURCE_DIR}")
   set(_kde5_${_dir}_${_target_name}_${_property_name} "${_property}")
endmacro (_KDE5_SET_CUSTOM_TARGET_PROPERTY)


macro (_KDE5_GET_CUSTOM_TARGET_PROPERTY _var _target_name _property_name)
   string(REGEX REPLACE "[/ ]" "_" _dir "${CMAKE_CURRENT_SOURCE_DIR}")
   set(${_var} "${_kde5_${_dir}_${_target_name}_${_property_name}}")
endmacro (_KDE5_GET_CUSTOM_TARGET_PROPERTY)


macro (KDE5_MOC_HEADERS _target_NAME)
   # if automoc4 from kdesupport has been found, use the macro provided there
   automoc4_moc_headers(${_target_NAME} ${ARGN})
endmacro (KDE5_MOC_HEADERS)

macro(KDE5_HANDLE_AUTOMOC _target_NAME _SRCS)
   # if automoc4 from kdesupport has been found, use the macro provided there
   automoc4(${_target_NAME} ${_SRCS})
endmacro(KDE5_HANDLE_AUTOMOC)

macro(KDE5_INSTALL_TS_FILES _lang _sdir)
...
endmacro(KDE5_INSTALL_TS_FILES)


macro (KDE5_CREATE_HANDBOOK _docbook)
   ...
   if (_kdeBootStrapping)
      ...
      set(_ssheet "${KDE5_DATA_INSTALL_DIR}/ksgmltools2/customization/kde-chunk.xsl")
...
#      set(KDE5_MEINPROC_EXECUTABLE meinproc4)
#   endif (CMAKE_CROSSCOMPILING)

   add_custom_command(OUTPUT ${_doc}
      COMMAND ${KDE5_MEINPROC_EXECUTABLE} --check ${_bootstrapOption} --cache ${_doc} ${_input}
      DEPENDS ${_docs} ${_KDE5_MEINPROC_EXECUTABLE_DEP} ${_ssheet}
   ...
   if(KDE5_ENABLE_HTMLHANDBOOK)
      set(_htmlDoc ${CMAKE_CURRENT_SOURCE_DIR}/index.html)
      add_custom_command(OUTPUT ${_htmlDoc}
         COMMAND ${KDE5_MEINPROC_EXECUTABLE} --check ${_bootstrapOption} -o ${_htmlDoc} ${_input}
         DEPENDS ${_input} ${_KDE5_MEINPROC_EXECUTABLE_DEP} ${_ssheet}
         WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
      )
      add_custom_target(htmlhandbook DEPENDS ${_htmlDoc})
   endif(KDE5_ENABLE_HTMLHANDBOOK)
...
endmacro (KDE5_CREATE_HANDBOOK)


macro (KDE5_CREATE_MANPAGE _docbook _section)
   get_filename_component(_input ${_docbook} ABSOLUTE)
   ...
   if (_kdeBootStrapping)
      ...
      set(_ssheet "${KDE5_DATA_INSTALL_DIR}/ksgmltools2/customization/kde-include-man.xsl")
      set(_bootstrapOption)
   endif (_kdeBootStrapping)
...
#   endif (CMAKE_CROSSCOMPILING)
   ...
   add_custom_command(OUTPUT ${_outdoc}
      COMMAND ${KDE5_MEINPROC_EXECUTABLE} --stylesheet ${_ssheet} --check ${_bootstrapOption} ${_input}
      DEPENDS ${_input} ${_KDE5_MEINPROC_EXECUTABLE_DEP} ${_ssheet}
   )
   get_filename_component(_targ ${CMAKE_CURRENT_SOURCE_DIR} NAME)
...
endmacro (KDE5_CREATE_MANPAGE)


macro (KDE5_UPDATE_ICONCACHE)
   ...
   install(CODE "
    ...
    ")
endmacro (KDE5_UPDATE_ICONCACHE)

# a "map" of short type names to the directories
# unknown names should give empty results
# KDE 3 compatibility
set(_KDE5_ICON_GROUP_mime       "mimetypes")
set(_KDE5_ICON_GROUP_filesys    "places")
set(_KDE5_ICON_GROUP_device     "devices")
set(_KDE5_ICON_GROUP_app        "apps")
set(_KDE5_ICON_GROUP_action     "actions")
# KDE 4 / icon naming specification compatibility
set(_KDE5_ICON_GROUP_mimetypes  "mimetypes")
set(_KDE5_ICON_GROUP_places     "places")
set(_KDE5_ICON_GROUP_devices    "devices")
set(_KDE5_ICON_GROUP_apps       "apps")
set(_KDE5_ICON_GROUP_actions    "actions")
set(_KDE5_ICON_GROUP_categories "categories")
set(_KDE5_ICON_GROUP_status     "status")
set(_KDE5_ICON_GROUP_emblems    "emblems")
set(_KDE5_ICON_GROUP_emotes     "emotes")
set(_KDE5_ICON_GROUP_animations "animations")
set(_KDE5_ICON_GROUP_intl       "intl")

# a "map" of short theme names to the theme directory
set(_KDE5_ICON_THEME_ox "oxygen")
set(_KDE5_ICON_THEME_cr "crystalsvg")
set(_KDE5_ICON_THEME_lo "locolor")
set(_KDE5_ICON_THEME_hi "hicolor")


# only used internally by KDE5_INSTALL_ICONS
macro (_KDE5_ADD_ICON_INSTALL_RULE _install_SCRIPT _install_PATH _group _orig_NAME _install_NAME _l10n_SUBDIR)

   # if the string doesn't match the pattern, the result is the full string, so all three have the same content
   if (NOT ${_group} STREQUAL ${_install_NAME} )
      set(_icon_GROUP  ${_KDE5_ICON_GROUP_${_group}})
      if(NOT _icon_GROUP)
...
endmacro (_KDE5_ADD_ICON_INSTALL_RULE)


macro (KDE5_INSTALL_ICONS _defaultpath )
   ...
   foreach (_current_ICON ${_icons} )
      ...
      set(_theme_GROUP ${_KDE5_ICON_THEME_${_type}})
      if( _theme_GROUP)
         _KDE5_ADD_ICON_INSTALL_RULE(${CMAKE_CURRENT_BINARY_DIR}/install_icons.cmake
...
# compiled all-in-one.
macro (KDE5_CREATE_FINAL_FILES _filenameCPP _filesExcludedFromFinalFile )
...
endmacro (KDE5_CREATE_FINAL_FILES)
...
# It overrides the defaults set in FindKDE5Internal.cmake.
...
macro (KDE5_HANDLE_RPATH_FOR_EXECUTABLE _target_NAME)
   if (UNIX)
      if (APPLE)
      ...
      if (APPLE)
         ...
         set(_ld_library_path "${LIBRARY_OUTPUT_PATH}/${CMAKE_CFG_INTDIR}/:${KDE5_LIB_DIR}")
      else (APPLE)
         set(_ld_library_path "${LIBRARY_OUTPUT_PATH}/${CMAKE_CFG_INTDIR}/:${LIB_INSTALL_DIR}:${KDE5_LIB_DIR}:${QT_LIBRARY_DIR}")
      endif (APPLE)
      ...
      add_custom_command(TARGET ${_target_NAME} POST_BUILD
         ...
         -P ${KDE5_MODULE_DIR}/kde5_exec_via_sh.cmake
         )
...
endmacro (KDE5_HANDLE_RPATH_FOR_EXECUTABLE)


macro (KDE5_ADD_PLUGIN _target_NAME )
...
#otherwise set the prefix empty
   ...
   _automoc4_kde5_pre_target_handling(${_target_NAME} _SRCS)
...
endmacro (KDE5_ADD_PLUGIN _target_NAME _with_PREFIX)
...
# (which evaluates to FALSE in cmake)
macro(KDE5_CHECK_EXECUTABLE_PARAMS _output_LIST _nogui _test)
   set(${_nogui})
   set(${_test})
...
endmacro(KDE5_CHECK_EXECUTABLE_PARAMS)


macro (KDE5_ADD_KDEINIT_EXECUTABLE _target_NAME )

   kde5_check_executable_params(_SRCS _nogui _test ${ARGN})

   configure_file(${KDE5_MODULE_DIR}/kde5init_dummy.cpp.in ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_dummy.cpp)
   ...
   if(WIN32)
      ...
      configure_file(${KDE5_MODULE_DIR}/kde5init_win32lib_dummy.cpp.in ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_win32lib_dummy.cpp)
      add_library(kdeinit_${_target_NAME} STATIC ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_win32lib_dummy.cpp)

      if (KDE5_ENABLE_FINAL)
         kde5_create_final_files(${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_final_cpp.cpp _separate_files ${_SRCS})
         kde5_add_executable(${_target_NAME} "${_nogui}" ${CMAKE_CURRENT_BINARY_DIR}/kdeinit_${_target_NAME}_final_cpp.cpp ${_separate_files} ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_dummy.cpp ${_resourcefile})

      else (KDE5_ENABLE_FINAL)
         kde5_add_executable(${_target_NAME} "${_nogui}" ${_SRCS} ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_dummy.cpp ${_resourcefile})
      endif (KDE5_ENABLE_FINAL)
      ...
      _automoc4_kde5_pre_target_handling(kdeinit_${_target_NAME} _SRCS)

      if (KDE5_ENABLE_FINAL)
         kde5_create_final_files(${CMAKE_CURRENT_BINARY_DIR}/kdeinit_${_target_NAME}_final_cpp.cpp _separate_files ${_SRCS})
      ...
      _automoc4_kde5_post_target_handling(kdeinit_${_target_NAME})

      set_target_properties(kdeinit_${_target_NAME} PROPERTIES OUTPUT_NAME kdeinit4_${_target_NAME})

      kde5_add_executable(${_target_NAME} "${_nogui}" ${CMAKE_CURRENT_BINARY_DIR}/${_target_NAME}_dummy.cpp)
      target_link_libraries(${_target_NAME} kdeinit_${_target_NAME})
   endif(WIN32)

endmacro (KDE5_ADD_KDEINIT_EXECUTABLE)
...
macro (KDE5_ADD_UNIT_TEST _test_NAME)
    set(_srcList ${ARGN})
    ...
    kde5_add_executable( ${_test_NAME} TEST ${_srcList} )

    if(NOT KDE5_TEST_OUTPUT)
        set(KDE5_TEST_OUTPUT plaintext)
    endif(NOT KDE5_TEST_OUTPUT)
    set(KDE5_TEST_OUTPUT ${KDE5_TEST_OUTPUT} CACHE STRING "The output to generate when running the QTest unit tests")
...
#    add_test( ${_targetName} ${EXECUTABLE_OUTPUT_PATH}/${_test_NAME} -xml -o ${_test_NAME}.tml )
    ...
    if (NOT MSVC_IDE)   #not needed for the ide
        ...
        endif (NOT KDE5_BUILD_TESTS)
    endif (NOT MSVC_IDE)

endmacro (KDE5_ADD_UNIT_TEST)
...
# This means that in KDE5_ADD_MANIFEST the LOCATION property contains 
...
# by _OUTPUT_NAME before calling kde5_add_executable as shown in the following example: 
# 
# set(xyz_OUTPUT_NAME test)
# kde5_add_executable( xyz <source>)
# set_target_properties( xyz PROPERTIES OUTPUT_NAME ${xyz_OUTPUT_NAME} )  
#
# The full solution would be to introduce a kde5_target_link_libraries macro and to 
# call KDE5_ADD_MANIFEST inside instead of calling in kde5_add_executable. 
...
# 
# This macro is an internal macro only used by kde5_add_executable
#
macro (_KDE5_ADD_MANIFEST _target_NAME)
    ...
    if (_kdeBootStrapping)
        ...
        set(_cmake_module_path ${KDE5_INSTALL_DIR}/share/apps/cmake/modules)
    endif (_kdeBootStrapping)
    ...
    add_custom_command(
        TARGET ${_target_NAME}
        POST_BUILD
        COMMAND ${KDE5_MT_EXECUTABLE}
        ARGS
           -manifest ${_manifest}
           -updateresource:${_executable}
        COMMENT "adding vista trustInfo manifest to ${_target_NAME}"
   )
endmacro(_KDE5_ADD_MANIFEST) 


macro (KDE5_ADD_EXECUTABLE _target_NAME)
   ...
   if (_test AND NOT KDE5_BUILD_TESTS)
      set(_add_executable_param ${_add_executable_param} EXCLUDE_FROM_ALL)
   endif (_test AND NOT KDE5_BUILD_TESTS)
   ...
   IF (KDE5_ENABLE_UAC_MANIFEST)
       _kde5_add_manifest(${_target_NAME})
   ...
   kde5_handle_rpath_for_executable(${_target_NAME})

   if (WIN32)
      target_link_libraries(${_target_NAME} ${QT_QTMAIN_LIBRARY})
   endif (WIN32)

endmacro (KDE5_ADD_EXECUTABLE)


macro (KDE5_ADD_LIBRARY _target_NAME _lib_TYPE)
...
endmacro (KDE5_ADD_LIBRARY _target_NAME _lib_TYPE)

macro (KDE5_ADD_WIDGET_FILES _sources)
   foreach (_current_FILE ${ARGN})
      ...
      add_custom_command(OUTPUT ${_source}
        COMMAND ${KDE5_MAKEKDEWIDGETS_EXECUTABLE}
        ARGS -o ${_source} ${_input}
        MAIN_DEPENDENCY ${_input} DEPENDS ${_KDE5_MAKEKDEWIDGETS_DEP})
...
endmacro (KDE5_ADD_WIDGET_FILES)


macro(KDE5_REMOVE_OBSOLETE_CMAKE_FILES)
...
endmacro(KDE5_REMOVE_OBSOLETE_CMAKE_FILES)
...
macro(KDE5_CREATE_EXPORTS_HEADER _outputFile _libName)
   ...
   configure_file(${KDE5_MODULE_DIR}/kde5exportsheader.h.in ${_outputFile})
endmacro(KDE5_CREATE_EXPORTS_HEADER _outputFile _libName)


macro (KDE5_CREATE_HTML_HANDBOOK _docbook)
   message(STATUS "KDE5_CREATE_HTML_HANDBOOK() is deprecated. Enable the option KDE5_ENABLE_HTMLHANDBOOK instead, this will give you targets htmlhandbook for creating the html help.")
endmacro (KDE5_CREATE_HTML_HANDBOOK)


# adds application icon to target source list
# 'appsources' - the sources of the application
# 'pngfiles' - specifies the list of icon files
# example: KDE5_ADD_WIN32_APP_ICON(myapp_SRCS "pics/cr16-myapp.png;pics/cr32-myapp.png")
macro (KDE5_ADD_WIN32_APP_ICON appsources)
    message(STATUS "KDE5_ADD_WIN32_APP_ICON() is deprecated, use KDE5_ADD_APP_ICON() instead")
    if (WIN32)
        if(NOT WINCE)
...
endmacro (KDE5_ADD_WIN32_APP_ICON)

# adds application icon to target source list 
# for detailed documentation see the top of FindKDE5Internal.cmake
macro (KDE5_ADD_APP_ICON appsources pattern)
...
macro (KDE5_HANDLE_RPATH_FOR_LIBRARY _target_NAME)
   message(STATUS "You are using the macro KDE5_HANDLE_RPATH_FOR_LIBRARY(), which is an internal macro and shouldn't be used by external projects. Please remove it.")
   if (NOT CMAKE_SKIP_RPATH)
      set_target_properties(${_target_NAME} PROPERTIES  SKIP_BUILD_RPATH FALSE  BUILD_WITH_INSTALL_RPATH FALSE)
   endif (NOT CMAKE_SKIP_RPATH)
endmacro (KDE5_HANDLE_RPATH_FOR_LIBRARY)
...
function(KDE5_INSTALL_AUTH_HELPER_FILES HELPER_TARGET HELPER_ID HELPER_USER)
    if(KDE5_AUTH_HELPER_BACKEND_NAME STREQUAL "DBUS")
        if (_kdeBootStrapping)
            set(_stubFilesDir  ${CMAKE_SOURCE_DIR}/kdecore/auth/backends/dbus/ )
        else (_kdeBootStrapping)
            set(_stubFilesDir  ${KDE5_DATA_INSTALL_DIR}/kauth/ )
        endif (_kdeBootStrapping)
    ...
    endif(KDE5_AUTH_HELPER_BACKEND_NAME STREQUAL "DBUS")
endfunction(KDE5_INSTALL_AUTH_HELPER_FILES)
...
# the install phase
function(KDE5_INSTALL_AUTH_ACTIONS HELPER_ID ACTIONS_FILE)

  if(KDE5_AUTH_BACKEND_NAME STREQUAL "APPLE")
    install(CODE "execute_process(COMMAND ${KDE5_KAUTH_POLICY_GEN_EXECUTABLE} ${ACTIONS_FILE} WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR})")
  elseif(KDE5_AUTH_BACKEND_NAME STREQUAL "POLKITQT" OR KDE5_AUTH_BACKEND_NAME STREQUAL "POLKITQT-1")
    set(_output ${CMAKE_CURRENT_BINARY_DIR}/${HELPER_ID}.policy)
    get_filename_component(_input ${ACTIONS_FILE} ABSOLUTE)

    add_custom_command(OUTPUT ${_output} 
                       COMMAND ${KDE5_KAUTH_POLICY_GEN_EXECUTABLE} ${_input} > ${_output} 
                       MAIN_DEPENDENCY ${_input}
                       WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
                       COMMENT "Generating ${HELPER_ID}.policy"
                       DEPENDS ${_KDE5_KAUTH_POLICY_GEN_EXECUTABLE_DEP})
    add_custom_target("actions for ${HELPER_ID}" ALL DEPENDS ${_output})

    install(FILES ${_output} DESTINATION ${KDE5_AUTH_POLICY_FILES_INSTALL_DIR})
  endif()

endfunction(KDE5_INSTALL_AUTH_ACTIONS)


macro(_KDE5_EXPORT_LIBRARY_DEPENDENCIES _append_or_write _filename)
   message(FATAL_ERROR "_KDE5_EXPORT_LIBRARY_DEPENDENCIES() was an internal macro and has been removed again. Just remove the code which calls it, there is no substitute.")
endmacro(_KDE5_EXPORT_LIBRARY_DEPENDENCIES)

macro (_KDE5_TARGET_LINK_INTERFACE_LIBRARIES _target _interface_libs)
   message(FATAL_ERROR "_KDE5_TARGET_LINK_INTERFACE_LIBRARIES() doesn't exist anymore. Set the LINK_INTERFACE_LIBRARIES target property instead. See kdelibs/kdecore/CMakeLists.txt for an example.")
endmacro (_KDE5_TARGET_LINK_INTERFACE_LIBRARIES)

macro (KDE5_TARGET_LINK_INTERFACE_LIBRARIES _target _interface_libs)
   message(FATAL_ERROR "KDE5_TARGET_LINK_INTERFACE_LIBRARIES() doesn't exist anymore. Set the LINK_INTERFACE_LIBRARIES target property instead. See kdelibs/kdecore/CMakeLists.txt for an example.")
endmacro (KDE5_TARGET_LINK_INTERFACE_LIBRARIES _target _interface_libs)

macro (KDE5_INSTALL_HANDBOOK _lang)
   message(STATUS "KDE5_INSTALL_HANDBOOK() is deprecated. Remove it please. Now all is done in KDE5_CREATE_HANDBOOK.")
endmacro (KDE5_INSTALL_HANDBOOK )

macro (KDE5_SET_CUSTOM_TARGET_PROPERTY)
   message(FATAL_ERROR "KDE5_SET_CUSTOM_TARGET_PROPERTY() is deprecated, just use a simple variable instead")
endmacro (KDE5_SET_CUSTOM_TARGET_PROPERTY)

macro (KDE5_GET_CUSTOM_TARGET_PROPERTY)
   message(FATAL_ERROR "KDE5_GET_CUSTOM_TARGET_PROPERTY() is deprecated, just use a simple variable instead")
endmacro (KDE5_GET_CUSTOM_TARGET_PROPERTY)
